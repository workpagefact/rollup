...
const bufferParsers: ((
	...
	function jsxFragment(node: JSXFragment, position, buffer, readString) {
		...
		node.children = convertNodeList(node, scope, buffer[position + 1], buffer, readString);
		node.closingFragment = convertNode(node, scope, buffer[position + 2], buffer, readString);
	},
	function jsxIdentifier(node: JSXIdentifier, position, buffer, readString) {
		node.name = convertString(buffer[position], buffer, readString);
	},
	function jsxMemberExpression(node: JSXMemberExpression, position, buffer, readString) {
		const { scope } = node;
		node.object = convertNode(node, scope, buffer[position], buffer, readString);
	...
	function jsxSpreadAttribute(node: JSXSpreadAttribute, position, buffer, readString) {
		const { scope } = node;
		node.argument = convertNode(node, scope, buffer[position], buffer, readString);
	},
	function jsxSpreadChild(node: JSXSpreadChild, position, buffer, readString) {
		const { scope } = node;
		node.expression = convertNode(node, scope, buffer[position], buffer, readString);
	},
